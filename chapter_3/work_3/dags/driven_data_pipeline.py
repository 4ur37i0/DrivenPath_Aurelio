...
def _write_to_csv() -> None:
    ...
    with open("/opt/airflow/data/raw_data.csv", mode="w", encoding="utf-8", newline="") as file: # Changed mode to "w" for fresh start
...
def _add_id() -> None:
    ...
    """
    # Forzar el tipo de 'personal_number' como texto
    df = pl.read_csv(
        "/opt/airflow/data/raw_data.csv",
        dtypes={
                "personal_number": pl.Utf8,
                "session_duration": pl.Int64 # Keep this as Int64 to help with type inference
            }, ignore_errors=True 
    )
...
def insert_csv_to_postgres():
    logging.info("Starting data insertion from CSV to PostgreSQL...")
    ...
    # Define the columns that are expected to be integers in the database
    integer_columns = [
        "session_duration",
        "download_speed",
        "upload_speed",
    ...
    with open("/opt/airflow/data/raw_data.csv", mode="r", encoding="utf-8") as file:
        reader = csv.DictReader(file)
        fieldnames = reader.fieldnames
        columns_sql = ", ".join([f'"{col}"' for col in fieldnames]) # Quote column names for safety

        for row in reader:
            row_values = []
            for col in fieldnames:
                value = row[col]
                # If the column is an integer type and the value is an empty string,
                # set it to None (which psycopg2 will convert to NULL)
                if col in integer_columns and value == "":
                    row_values.append(None)
                else:
                    row_values.append(value)
            
            placeholders = ", ".join(["%s"] * len(row_values))
            query = f"INSERT INTO driven_raw.raw_batch_data ({columns_sql}) VALUES ({placeholders})"
            
            try:
                cur.execute(query, row_values)
            except psycopg2.Error as e:
                logging.error(f"Error inserting row: {row_values}. Error: {e}")
